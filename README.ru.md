...
ocyd находится на ранней альфа-стадии, поэтому поддержка моделей ещё расширяется.
...
Откройте страницу [Releases](https://github.com/Dilikel/ocyd/releases), скачайте архив исходников последнего релиза, распакуйте его и запустите инсталлятор из распакованной директории:
...
Имя архива может отличаться в разных релизах. Используйте имя архива и директории, указанные на странице Releases.
...
Чтобы удалить установленный демон, пользовательский сервис и udev-правило, запустите `uninstall.sh` из директории проекта или распакованного архива:

```bash
./uninstall.sh
```

Скрипт остановит и отключит сервис, удалит бинарники из `/usr/local/bin`, удалит `/etc/udev/rules.d/99-ocypus.rules` и перезагрузит правила udev. Скрипт спросит, нужно ли удалить `~/.config/ocyd`, а для удаления системных файлов потребуется `sudo`.
...
## Управление демоном вручную

Демон запускается как пользовательский systemd-сервис:
...
Чтобы снова включить автозапуск, выполните `systemctl --user enable ocyd.service`.
